...
ggplot(environment, aes(brickType, biomass, color = brickRatio)) +
  geom_boxplot() +
  geom_quasirandom(dodge.width = .7)
#2way (brickType:f.watering):
ggplot(environment, aes(f.watering, biomass, color = brickType)) +
  geom_boxplot() +
  geom_quasirandom(dodge.width = .7)
#2way (brickType:seedmix):
ggplot(environment, aes(seedmix, biomass, color = brickType)) +
  geom_boxplot() +
  geom_quasirandom(dodge.width = .7)
...
dotchart((environment$biomass),
         groups = factor(environment$brickType), main = "Cleveland dotplot")
dotchart((environment$biomass),
...
write.csv(tidytable, here("outputs", "statistics",
                          "table_anova_experiment_2_biomass.csv"))
