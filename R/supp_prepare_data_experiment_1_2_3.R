...
### Load data ###
data <- read_csv("supp_data_raw_experiment_1_2_3.csv", col_names = T, na = "na", col_types = 
                       cols(
                         .default = "d"
                       )) %>%
...
data1 <- data %>%
  filter(substrate == "substrate_7" | substrate == "substrate_8" | substrate == "substrate_9" | substrate == "substrate_10") %>%
...
data2 <- data %>%
  filter(substrate == "substrate_7" | substrate == "substrate_8" | substrate == "substrate_11" | substrate == "substrate_12") %>%
  mutate(substrateAbb = dplyr::recode(substrate,
...
data3 <- data %>%
  filter(substrate == "substrate_13" | substrate == "substrate_14" | substrate == "substrate_15" | substrate == "substrate_16" | substrate == "substrate_17" | substrate == "substrate_18") %>%
  mutate(substrateAbb = dplyr::recode(substrate,
...
### Save processed data ###
write_csv(data1, here("data/processed/supp_data_processed_experiment_1.csv"))
write_csv(data2, here("data/processed/supp_data_processed_experiment_2.csv"))
write_csv(data3, here("data/processed/supp_data_processed_experiment_3.csv"))
