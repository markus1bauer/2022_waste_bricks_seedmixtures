...
ggplot(environment, aes(brickRatio, biomass, color = acid)) +
  geom_boxplot() +
...
ggplot(environment, aes(seedmix, biomass, color = brickRatio)) +
  geom_boxplot() +
  geom_quasirandom(dodge.width = .7)
...
ggplot(environment, aes(f.watering, biomass, color = seedmix)) +
  ...
  geom_quasirandom(dodge.width = .7)
#2way (acid:watering):
ggplot(environment, aes(f.watering, biomass, color = acid)) +
  geom_boxplot() +
  geom_quasirandom(dodge.width = .7)
#3way (brickRatio:acid:seedmix):
ggplot(environment, aes(brickRatio, biomass, color = acid)) +
  facet_grid(~seedmix) + geom_boxplot() +
  geom_quasirandom(dodge.width = .7)
...
ggplot(environment, aes(brickRatio, biomass, color = acid)) +
  ...
  geom_boxplot() +
  geom_quasirandom(dodge.width = .7)
#4way (focus only on standard and intermediate)
ggplot(environment,
       aes(f.watering, biomass, color = brickRatio, shape = acid)) +
  facet_grid(~seedmix) +
  geom_boxplot() +
  geom_quasirandom(dodge.width = .7)
...
ggplot(environment, aes(brickRatio, biomass, color = acid)) +
  ...
  facet_wrap(~block) +
  geom_quasirandom(dodge.width = .7)
ggplot(environment, aes(block, biomass, color = f.watering)) +
  geom_boxplot() +
  geom_quasirandom(dodge.width = .7)
ggplot(environment, aes(block, biomass, color = seedmix)) +
  geom_boxplot() +
  geom_quasirandom(dodge.width = .7)
...
dotchart((environment$biomass),
         groups = factor(environment$acid), main = "Cleveland dotplot")
dotchart((environment$biomass),
         groups = factor(environment$watering), main = "Cleveland dotplot")
dotchart((environment$biomass),
         groups = factor(environment$seedmix), main = "Cleveland dotplot")
dotchart((environment$biomass),
         groups = factor(environment$grassRatio), main = "Cleveland dotplot")
par(mfrow = c(1, 1))
boxplot(environment$biomass, ylim = c(0, 45))
par(mfrow = c(2, 2))
plot(table((environment$biomass)), type = "h",
...
plotResiduals(main = "acid", simulationOutput$scaledResiduals,
              environment$acid)
...
### Effect sizes --------------------------------------------------------------
(emm <- emmeans(m5, revpairwise ~ seedmix | f.watering,
                type = "response"))
plot(emm, comparison = TRUE)
contrast(emmeans(m5, ~ seedmix * f.watering,
                 type = "response"),
         "trt.vs.ctrl", ref = 1)
...
write.csv(tidytable, here("outputs", "statistics",
                          "table_anova_experiment_1_biomass.csv"))
